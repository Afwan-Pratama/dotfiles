...
ColumnLayout {
  ...
  ColumnLayout {
    ...
    NCollapsible {
      ...
      NCheckbox {
        label: "Neovim"
        description: ProgramCheckerService.weztermAvailable ? I18n.tr("settings.color-scheme.templates.terminal.neovim.description", {
                                                                        "filepath": "~/.config/nvim/lua/custom/plugins/base16.lua"
                                                                      }) : I18n.tr("settings.color-scheme.templates.terminal.neovim.description-missing", {
                                                                                     "app": "neovim"
                                                                                   })
        checked: Settings.data.templates.neovim
        enabled: ProgramCheckerService.neovimAvailable
        opacity: ProgramCheckerService.neovimAvailable ? 1.0 : 0.6
        onToggled: checked => {
                     if (ProgramCheckerService.neovimAvailable) {
